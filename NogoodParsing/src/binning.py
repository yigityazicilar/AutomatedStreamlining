...
def bin_instance(
    ...
    binned_result: Dict[str, np.ndarray] = {}
    for variable_name, arr in find_bins.items():
        binning_guide = map_values_to_nd_bins(arr, number_of_bins)
        binned_arr = apply_binning(binning_guide, arr, number_of_bins)
        binned_arr_sum = np.sum(binned_arr)

        if binned_arr_sum != 0:
            binned_arr = (
                binned_arr / binned_arr_sum * 100
            )  # Sum normalize, convert to percentage

        binned_result[variable_name] = binned_arr
